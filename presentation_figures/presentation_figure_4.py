...
# example.lcdm_shear_2params_param_names = ['log_omegam', 'log_sigma8']
# log_params_flow_cache = out_folder+'log_params_flow_cache_shear'
# temp = DES_generate.helper_load_chains(example.lcdm_shear_2params_param_names, example.prior_chain, example.posterior_chain_lcdm_shear, log_params_flow_cache, feedback=0)
# example.lcdm_shear_2params_prior_flow, example.lcdm_shear_2params_posterior_flow = temp
#
# example.lcdm_shear_2params_param_names = ['omegam', 'sigma8']
# params_flow_cache = out_folder+'params_flow_cache_shear'
# temp = DES_generate.helper_load_chains(example.lcdm_shear_2params_param_names, example.prior_chain, example.posterior_chain_lcdm_shear, params_flow_cache)
# params_prior_flow, example.lcdm_shear_2params_posterior_flow = temp

###############################################################################
# do Local PCA:
num_params = len(example.lcdm_shear_2params_param_names)

reference_point = example.posterior_chain_lcdm_shear.getMeans(pars=[example.posterior_chain_lcdm_shear.index[name] for name in example.lcdm_shear_2params_log_param_names])

# local fisher:
fisher = example.lcdm_shear_log_2params_posterior_flow.metric(example.lcdm_shear_log_2params_posterior_flow.cast([reference_point]))[0]
prior_fisher = example.lcdm_shear_log_2params_prior_flow.metric(example.lcdm_shear_log_2params_prior_flow.cast([reference_point]))[0]

eig, eigv = utilities.KL_decomposition(fisher, prior_fisher)
sqrt_fisher = scipy.linalg.sqrtm(fisher)

# solve for modes:
m1, m2 = np.exp(example.posterior_chain_lcdm_shear.getMeans([example.posterior_chain_lcdm_shear.index[name] for name in example.lcdm_shear_2params_log_param_names]))
y0 = example.lcdm_shear_2params_posterior_flow.cast([m1,m2])#example.posterior_chain_lcdm_shear.getMeans(pars=[example.posterior_chain_lcdm_shear.index[name] for name in example.lcdm_shear_2params_param_names]))

length_1 = 10
length_2 = 15

_, LKL_mode_1, _ = synthetic_probability.solve_KL_ode(example.lcdm_shear_2params_posterior_flow, example.lcdm_shear_2params_prior_flow, y0, n=1, length=length_1, num_points=1000)
_, LKL_mode_2, _ = synthetic_probability.solve_KL_ode(example.lcdm_shear_2params_posterior_flow, example.lcdm_shear_2params_prior_flow, y0, n=0, length=length_2, num_points=1000)
...
density_1 = example.posterior_chain_lcdm_shear.get2DDensity(example.lcdm_shear_2params_param_names[0], example.lcdm_shear_2params_param_names[1], normalized=True)
...
# ticks:
ticks = [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
...
plt.savefig(out_folder+'/figure_DES_LCDM_single_1.pdf')
plt.close('all')
