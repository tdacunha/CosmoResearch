...
num_params = len(example.lcdm_shear_params_param_names)
# reference point:
reference_point = np.array([name.best_fit for name in example.posterior_chain_lcdm_shear.getBestFit().parsWithNames(example.lcdm_shear_params_param_names)])
reference_point = np.array([example.posterior_chain_lcdm_shear.samples[np.argmin(example.posterior_chain_lcdm_shear.loglikes), :][example.posterior_chain_lcdm_shear.index[name]] for name in example.lcdm_shear_params_param_names])
reference_point = example.posterior_chain_lcdm_shear.getMeans(pars=[example.posterior_chain_lcdm_shear.index[name] for name in example.lcdm_shear_params_param_names])
# solve for modes:
y0 = example.lcdm_shear_params_posterior_flow.cast(reference_point)
length_1 = (example.lcdm_shear_params_posterior_flow.sigma_to_length(5)).astype(np.float32)
length_2 = (example.lcdm_shear_params_posterior_flow.sigma_to_length(5)).astype(np.float32)
...
# save:
g.fig.savefig(out_folder+'/figure_14p2.pdf')
