...
try:
    ...
    #tf.config.set_visible_devices([], 'GPU')  # minsup
...
class DiffFlowCallback(Callback):
    ...
    * `Z` designates samples in the gaussianized space, connected to `Y` with a normalizing flow denoted `Z2Y_bijector`.
    ...
    def __init__(self, diff_chain, param_names=None, Z2Y_bijector='MAF', pregauss_bijector=None, learning_rate=1e-3, feedback=1, validation_split=0.1, early_stop_nsigma=0., early_stop_patience=10, **kwargs):
        ...
        covmat = np.array(np.diag(np.linalg.eigvals(diff_chain.cov(pars=param_names))), dtype = "float32")
        self._init_transf_dist(Z2Y_bijector,covmat, learning_rate=learning_rate, **kwargs)
        if feedback > 0:
            print("Building flow")
    ...
    def _init_transf_dist(self, Z2Y_bijector, covmat, learning_rate=1e-4, **kwargs):
        # Model
        if Z2Y_bijector == 'MAF':
        ...
        #self.dist_transformed = tfd.TransformedDistribution(distribution=tfd.MultivariateNormalFullCovariance(np.zeros(self.num_params, dtype=np.float32), covmat), bijector=Z2Y_bijector)
        ...
        # Full distribution
        #self.dist_learned = tfd.TransformedDistribution(distribution=tfd.MultivariateNormalFullCovariance(np.zeros(self.num_params, dtype=np.float32), covmat), bijector=self.Z2X_bijector)  # samples from std gaussian mapped to X
    ...
    def _plot_loss(self, ax, logs={}):
        ...
        if ax is not None:
            ax.loglog(self.log["loss"], label='Training')
            ax.loglog(self.log["val_loss"], label='Testing')
