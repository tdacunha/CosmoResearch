...
# output folder:
out_folder = './results/example_DES_shear_wcdm/'
...
prior_chain = getdist.mcsamples.loadMCSamples(file_root=DES_generate.chains_dir+'004_DESY1_shear_wCDM_prior', no_cache=True, settings=DES_generate.settings)
posterior_chain = getdist.mcsamples.loadMCSamples(file_root=DES_generate.chains_dir+'004_DESY1_shear_wCDM', no_cache=True, settings=DES_generate.settings)
...
class temp(bj.Identity):
    @property
    def name(self):
        return ''


transformation = [bj.Log(), bj.Log(), bj.Log(), bj.Log(), bj.Log(), temp()]
