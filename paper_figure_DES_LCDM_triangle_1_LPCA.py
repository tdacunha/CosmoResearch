...
_, LPCA_mode_1 = example.params_posterior_flow.solve_eigenvalue_ode_par(y0, n=0, length=length_1, num_points=1000)
_, LPCA_mode_2 = example.params_posterior_flow.solve_eigenvalue_ode_par(y0, n=1, length=length_2, num_points=1000)
_, LPCA_mode_3 = example.params_posterior_flow.solve_eigenvalue_ode_par(y0, n=2, length=length_3, num_points=1000)
...
for i in range(num_params-1):
    for i2 in range(bottom, i-1, -1):
        ...
        ax.plot(LPCA_mode_1[:, idx1], LPCA_mode_1[:, idx2], c=colors[1], lw=1., ls='-', zorder=998, label='LPCA mode 1')
        ax.plot(LPCA_mode_2[:, idx1], LPCA_mode_2[:, idx2], c=colors[2], lw=1., ls='-', zorder=998, label='LPCA mode 2')
        ax.plot(LPCA_mode_3[:, idx1], LPCA_mode_3[:, idx2], c=colors[3], lw=1., ls='-', zorder=998, label='LPCA mode 3')
...
# save:
g.fig.savefig(out_folder+'/figure_DES_LCDM_triangle_1_LPCA.pdf')
